const ConsistencyRing = () => {
  ...
  const totalDays = 25; // days so far this month
  ...
  return (
    <div className="flex items-center gap-4 bg-card rounded-2xl p-4 card-shadow animate-scale-in">
      ...
      <div>
        <p className="text-sm font-semibold text-foreground">Consistencia del mes</p>
        <p className="text-xs text-muted-foreground mt-0.5">
          {daysRegistered} de {totalDays} días registrados
        </p>
        <p className="text-xs text-primary font-medium mt-1">¡Sigue así! 💪</p>
      </div>
    </div>
  );
};
